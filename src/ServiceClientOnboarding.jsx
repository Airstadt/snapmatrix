...
export default function ServiceClientOnboarding({ setView }) {
  const [view, setInternalView] = useState("form"); 

  useEffect(() => {
    document.title = "Client Onboarding Tool | Demo Mode";
  }, []);
  ...
  async function generate() {
    ...
    try {
      ...
      // --- DATABASE WRITE DISABLED FOR DEMO MODE ---
      /*
      ...
      */
      console.log("Demo Mode: Data not saved to Firebase.");
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", background: "#f0f2f5", padding: "40px 20px", fontFamily: "sans-serif" }}>
      ...
      <div style={{ position: "fixed", bottom: "30px", right: "30px", zIndex: 1000, display: "flex", flexDirection: "column", gap: "10px" }}>
        <button onClick={() => setView("production")} style={{ padding: "12px 24px", background: view === "production" ? "#5b21b6" : colors.prodPurple, color: "white", border: "none", borderRadius: "50px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 8px 20px rgba(0,0,0,0.15)" }}>🎬 Production</button>
        <button onClick={() => setView("inventory")} style={{ padding: "12px 24px", background: view === "inventory" ? "#1e40af" : colors.infoBlue, color: "white", border: "none", borderRadius: "50px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 8px 20px rgba(0,0,0,0.15)" }}>📋 Inventory</button>
        <button onClick={() => setView("receiving")} style={{ padding: "12px 24px", background: view === "receiving" ? "#059669" : colors.successGreen, color: "white", border: "none", borderRadius: "50px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 8px 20px rgba(0,0,0,0.15)" }}>🚚 Receiving</button>
        <button onClick={() => setView("po")} style={{ padding: "12px 24px", background: view === "po" ? colors.secondary : colors.primary, color: "white", border: "none", borderRadius: "50px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 8px 20px rgba(0,0,0,0.15)" }}>📦 Create P.O.</button>
        
        <button 
          onClick={() => setInternalView(view === "form" ? "dash" : "form")}
          style={{ padding: "12px 24px", background: colors.primary, color: "white", border: "none", borderRadius: "50px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 8px 20px rgba(0,0,0,0.15)" }}
        >
      ...
      {view === "form" ? (
        <div style={{ maxWidth: "800px", margin: "0 auto" }}>
          <header style={{ textAlign: "center", marginBottom: "30px" }}>
            <h1 style={{ color: colors.primary, fontSize: "32px", fontWeight: "800", margin: 0 }}>Client Onboarding Service</h1>
            <p style={{ color: "#64748b", marginTop: "10px", marginBottom: "20px" }}>Demo Mode: Practice dispatching jobs without saving data.</p>
          </header>
          ...
          <div style={{ background: "white", padding: "30px", borderRadius: "20px", boxShadow: "0 10px 25px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button onClick={generate} disabled={loading} style={{ flex: 2, padding: "16px", background: colors.primary, color: "white", border: "none", borderRadius: "10px", fontWeight: "bold", cursor: "pointer", opacity: loading ? 0.6 : 1 }}>
                {loading ? "Generating Brief..." : "Generate Demo Brief"}
              </button>
        ...
        <POModule setView={setView} />
      ) : view === "receiving" ? (
        <ReceivingModule setView={setView} />
      ) : view === "inventory" ? (
        <InventoryDashboard setView={setView} />
      ) : view === "production" ? (
        <ProductionOnboarding setView={setView} />
      ) : null}
    </div>
  );
}